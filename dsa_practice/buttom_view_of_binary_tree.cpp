...
vector<int> buttomView(node *root){
    ...
    while(!q.empty()){
        ...
        m[horizontal_height_of_temp] = temp->data;		//we have to keep updating wether height is 0 or not 
														//Because we want the last element with same horizontal height
        if(temp -> left){
...
int main()
{
    ...
    vector <int> res = buttomView(root);
